...
class RequestHandler(webapp.RequestHandler):
  def action_url(self, action):
    return '%s/turkadmin/action?key=%s' % (self.request.host_url, action.key())

  def operation_url(self, operation):
    return '%s/turkadmin/operation?key=%s' % (self.request.host_url, operation.key())

  def operation_task_url(self, operation):
    return '%s/turkadmin/operation/task?key=%s' % (self.request.host_url, operation.key())
